use antelope_firewall_lib::{
    config::{from_config, Config},
    firewall_builder::{AntelopeFirewall, AntelopeFirewallError, RoutingModeState},
};
use clap::Parser;
...
async fn main() {
    ...
    match &std::fs::read_to_string(args.config.clone()) {
        Ok(contents) => match toml::from_str::<Config>(contents) {
            Ok(parsed) => match from_config(parsed).await {
                Ok(firewall) => {
                    let res = firewall.build().run().await;
                    if let Err(e) = res {
                        error!(
                            "Error occurred while running firewall. Received error: {}",
                            e
                        );
                    }
                }
                Err(e) => {
                    error!(
                        "Error occurred while configuring firewall. Received error: {}",
                        e
                    );
                }
            },
